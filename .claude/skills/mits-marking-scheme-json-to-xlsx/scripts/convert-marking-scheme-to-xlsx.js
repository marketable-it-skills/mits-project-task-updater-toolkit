 ...
 * Usage: node convert-marking-scheme-to-xlsx.js <input.json> <subcriterionPrefix> [output.xlsx] [--day=N]
 *   subcriterionPrefix: first letter for Sub Criterion IDs, e.g. B → B1, B2, …
 *   --day=N: Day of Marking written into column C (default 1).
 * WSOS Section (column I) is taken from each aspect's wsosSection (fallback 1);
 * judgement level rows leave column I empty.
 */
...
/** Day of marking (column C) — from --day=N, default 1 */
const DAY_OF_MARKING = dayArg ? parseInt(dayArg.split("=")[1], 10) : 1;
...
if (!inputPath || prefixArg === undefined) {
  ...
  console.error("  --day=N: Day of Marking for column C (default 1)");
  process.exit(1);
}

if (Number.isNaN(DAY_OF_MARKING)) {
  console.error("--day must be an integer, e.g. --day=2");
  process.exit(1);
}
      ...
      if (a.type === "measurement") {
        ...
        for (let i = 0; i < 4; i++) {
          rows.push([
            ...
            "", "", "", "", // H-K empty (col I empty on level rows)
          ]);
        }
      }
    });
