- Read the `/current-project.md` to identify the project task (test project) folder we are working on. All folders and files below are relative to this directory except when the path starts with "/" which indicates a full path.
...
2. **Reference Framework Study**:
   - Use `references/marking-scheme-creation-guideline.md` as the primary guideline for structure and standards
   - Analyze similar marking schemes from the prevously created project tasks in`/project-tasks` folder. The marking scheme can be found in the `marking/marking-scheme.json`in each project folder. Identify the project tasks which match the module type of the current project task and use their marking scheme as reference.
...
4. **Logical Grouping and Hierarchy**:
   ...
   - Typical groupings by module type (use these only as reference but be creative to create groups which suite the project task):
     - **Static websites** (Design implementation, HTML, CSS): Project Structure, Content Communication, Visual Design, Technical Implementation
     - **Dynamic websites** (SSR, admin interface): Database Design, Authentication/Security, Administrative Interface, Server-Side Rendering
     - **REST API Backend**: Authentication, API Endpoints, Error Handling, External Integration
     - **REST API Interactive forntend**: Frontend Design, User Interface, API Integration, Advanced Features
     - **Non-REST API Interactive Frontend** (e.g JS Game, web app prototype ): Layout, Data handling, User Experience and Clean Code
...
5. **WSOS Section Mapping**:
   - Assign each aspect to appropriate WSOS section (1-3):
...
6. **Strategic Point Distribution**:
   - **Total Points**: Check project-description.md for "Mark distribution" section first. If not found, ask the user for it.
